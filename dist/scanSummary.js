...
exports.buildFullScanMessage = buildFullScanMessage;
exports.buildScanResultsImage = buildScanResultsImage;
...
function dedupeSortedRows(rows) {
    const seen = new Set();
    const result = [];
...
function buildFullScanMessage(rows, trigger = "manual") {
    if (!rows.length) {
        ...
        if (!section.rows.length) {
            ...
            for (const [dateIndex, date] of [...grouped.keys()].sort().entries()) {
                ...
                for (const row of grouped.get(date) ?? []) {
                    ...
                    lines.push(`${row.origin}->${row.destination} | ${PRICE_BAND_COLORS[row.price_band?.toLowerCase()] ?? "AZUL"} ${row.outbound_date} | ${row.price_fmt}${vendorText}${bestNote}`);
                }
...
function renderRows(rows, title, highlightField, badgeClass) {
    if (!rows.length) {
        return `
...
async function buildScanResultsImage(rows, title = "Consulta completa") {
  ...
  `;
    ...
    try {
        ...
        const imagePath = node_path_1.default.join(node_os_1.default.tmpdir(), `telegram-scan-${Date.now()}.png`);
