...
HOSTINGER_PLAYWRIGHT_BROWSERS_PATH="${HOSTINGER_PLAYWRIGHT_BROWSERS_PATH:-${HOSTINGER_REMOTE_DIR}/.playwright-browsers}"
...
Opcionais:
  ...
  HOSTINGER_PLAYWRIGHT_BROWSERS_PATH=/caminho/remoto/.playwright-browsers
...
if [[ "${HOSTINGER_INSTALL_PLAYWRIGHT}" == "1" ]]; then
  REMOTE_CMD+=" && mkdir -p '${HOSTINGER_PLAYWRIGHT_BROWSERS_PATH}'"
  REMOTE_CMD+=" && PLAYWRIGHT_BROWSERS_PATH='${HOSTINGER_PLAYWRIGHT_BROWSERS_PATH}' ${HOSTINGER_NPX_BIN} playwright install chromium"
fi
