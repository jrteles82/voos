import { AIRPORT_OPTIONS, appConfig } from "./config";

function optionsHtml(selected: string): string {
  return AIRPORT_OPTIONS.map(([code, label]) => `<option value="${code}"${code === selected ? " selected" : ""}>${label}</option>`).join("");
}
...
export function renderPanel(input: {
  user: { email: string };
  routes: Array<{ id: number; origin: string; destination: string; outbound_date: string; inbound_date: string }>;
  tg?: { bot_token: string; chat_id: string };
        ...
        <div class="p-4">
          ...
          <div class="card shadow-sm dashboard-section d-none" id="cron">
            ...
            <div class="card-body">
              ...
              <div class="small text-muted mt-2">${appConfig.restartCommand ? "O painel usará SKYSCANNER_RESTART_COMMAND." : "Nenhum comando de reinício configurado."}</div>
            </div>
          </div>
        </div>
      </main>
